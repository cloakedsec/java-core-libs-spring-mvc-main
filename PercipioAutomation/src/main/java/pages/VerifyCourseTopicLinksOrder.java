...
public class VerifyCourseTopicLinksOrder {
	...
	public static void main(String[] args) throws Exception {
		...
		WebElement topicLink = driver.findElement(By.xpath("//a[@data-marker=\"CourseContentsCardTitleLink0\"]"));

		WebElement topicOrder = driver.findElement(By.xpath("//td[@class=\"middle\" and contains(text(), '')]"));
		...
		for (int rowNo = 20; rowNo < 27; rowNo++) {
			if (sheet7.getRow(rowNo).getCell(1).getRichStringCellValue().getString().trim().equals(xpathUpdate))
				;
			System.out.println("Link" + count++ + " Verified and Matched !!!");
